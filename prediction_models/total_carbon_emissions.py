...
df = pd.read_excel('finaldata.xlsx', sheet_name=0, usecols=['Year','Total'])
ts = df['Total']
...
fcast = model_fit.get_prediction(start=1, end=150)
...
while(i<150):
	print(ts_p[i+1])
	i = i + 1
workbook = xlsxwriter.Workbook('carbon_emissions_predictions.xlsx')
worksheet = workbook.add_worksheet()
i = 0
c = 0
data = []
while(i <= 150 and c <= 149) :
	data.append([1950 + i,ts_p[i+1]])
	i = i  + 1
	c = c + 1
